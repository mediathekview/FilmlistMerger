...
public class FilmlistOldFormatReader2 {
  private static final String ENTRY_DELIMETER = "\\],";
  private static final Logger LOG = LogManager.getLogger(FilmlistOldFormatReader2.class);
    ...
    for (final List<String> splittedEntry : splittedEntries) {
      if (isFirst) {
        ...
        filmEntryBefore =
          ...
          LOG.info(String.format("unkown entry %s",splittedEntry));
      }
    }
    return futureFilms;
  }
  ...
  private void setMetaInfo(final Filmlist aFilmlist, final List<String> aSplittedEntry) {
    try {
      ...
      LOG.warn("Somethin went wrong on setting the meta data of filmlist.", exception);
    }
  }
