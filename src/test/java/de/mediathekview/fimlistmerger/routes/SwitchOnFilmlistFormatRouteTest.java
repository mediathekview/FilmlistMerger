...
class SwitchOnFilmlistFormatRouteTest {
  ...
  void setUp() throws Exception {
    ...
    Files.copy(Paths.get(ClassLoader.getSystemResource("input/TestFilmlistOld.json").toURI()), tempInputFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
  }
  ...
  private void setUpRouteUnderTest() throws Exception {
    AdviceWith.adviceWith(
        ...
        advice -> {
          ...
          advice.weaveById(InputFilesRoute.NEW_FILM_FORMAT_ROUTING_TARGET).replace().to(mockOldFormatEndpoint);
        });
  }
